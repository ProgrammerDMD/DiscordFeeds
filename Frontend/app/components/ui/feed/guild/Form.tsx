...
import { deleteFeed, getChannels, processForm } from "@/app/utils/GuildServerActions";
import { Channel, FeedType } from "@/types/APITypes";
...
export default function Form({ guildId }: {
    guildId: string,
}) {
    const { options, visible, showForm } = useFormStore();

    const { register, watch, handleSubmit, formState, reset, setError, clearErrors, getValues } = useForm<GuildFormSchemaType>({
        defaultValues: {
            name: "",
            channel: "select",
            interval: "300"
    ...
    useEffect(() => {
        console.log(options?.jobDetails);
        reset((options !== undefined) ? {
            name: options.jobDetails.name,
            channel: options.jobDetails.discord_channel,
    ...
    const onSubmit: SubmitHandler<GuildFormSchemaType> = async (data) => {
        await processForm({
            id: options?.id,
            guild: guildId,
            form: data
    ...
    useEffect(() => {
        async function deleteFeedFunc() {
            if (isDeleting && options?.id) {
                await deleteFeed(options?.id, guildId);
                setDeleting(false);
    ...
    useEffect(() => {
        async function fetcher() {
            getChannels(guildId).then((value) => {
                setChannels(value);
            });
        }
    ...
    return visible && <div className="absolute m-auto w-screen h-screen z-10 bg-gray bg-opacity-20">
        <div className="overflow-y-auto flex flex-col p-5 absolute top-0 left-0 right-0 bottom-0 w-fit max-w-[80%] h-fit max-h-[80%] m-auto justify-center bg-white rounded-2xl gap-2 transition-all ease-in shadow-custom-form">
            ...
            <form id="guild-form" className={`overflow-y-auto px-4 py-2 mb-2 flex flex-col gap-5 text-center text-xl ${encodesans.className}`} onSubmit={handleSubmit(onSubmit)} >
                <div>
                ...
                <div>
                    <label htmlFor="channel">In what text channel do you want to receive this feed?</label>
                    <div className="bg-white rounded-3xl">
                        <select id="channel" {...register("channel")} className="px-4 py-2 w-full font-bold">
                            <option key="select" value="select" hidden={true}>No channel selected</option>
                            {!!channels && channels.map(function (channel) {
                                return <option key={channel.id} value={channel.id}>#{channel.name} - {channel.id} </option>
                            })}
                        </select>
                    </div>
                </div>
            ...
            <div className="flex flex-row flex-wrap items-center align-middle gap-2 justify-center">
                <button form="guild-form" disabled={isSubmitting || isDeleting} type="submit" className="flex w-max items-center px-5 py-2 rounded-full bg-blue transition-colors duration-100 ease-linear hover:bg-opacity-80 hover:disabled:bg-opacity-50 disabled:bg-opacity-50 cursor-pointer">
                    <span className={`${koulen.className} text-white text-[1.5em] cursor-pointer`}>Submit</span>
                </button>
                <button form="guild-form" disabled={isSubmitting || isDeleting} type="reset" onClick={() => {
                    reset({});
                    ...
                    <span className={`${koulen.className} text-white text-[1.5em] cursor-pointer`}>Cancel</span>
                </button>
                {options !== undefined &&
                    <button form="guild-form" disabled={isSubmitting || isDeleting} type="reset" onClick={() => {
                        setDeleting(true);
